"""Some auxiliary files used for classwork assignment"""
...
def load_dataset():
    ...
    encoder = OneHotEncoder(sparse=False).fit(y_train[:,None])

    y_train = encoder.transform(y_train[:,None])
    y_test = encoder.transform(y_test[:,None])
